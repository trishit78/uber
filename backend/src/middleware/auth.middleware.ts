...
export async function isDriverAuthenticated(token:string){
    try {
        ...
        const user = await getDriverById((response as any).id);
...
export const driverAuthRequest = async(req:Request,res:Response,next:NextFunction)=>{
    try {
        const authHeader =req.headers.authorization;
        if(!authHeader || !authHeader.startsWith('Bearer')){
            res.status(401).json({
        ...
        const response =await isDriverAuthenticated(token);
        if(!response){
            res.status(401).json({
                success:false,
                message:'Unauthorized'
